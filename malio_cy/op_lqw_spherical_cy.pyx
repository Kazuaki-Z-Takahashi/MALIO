...
def calc_q_norm(l_sph, q_i_func):
    ...
    # norm2 = math.sqrt( norm2* np.conjugate(norm2))
...
def calc_spherical_order_parameter(calc_type, coord, direct, box_length, setting, neighbor_list, thread_num):
    ...
    cdef:
        ...
        double dist, theta, phi, sum_norm2, sum_vec
        ...
        double a_ij, q_i_norm, q_j_norm, a_temp
        cvec q_i_func = cvec()
        cvec q_j_func = cvec()
        cvec q_ij_func = cvec()
            ...
            for b_t in range(b_times + 1):
                for o_f, o_i, o_j, p_weight in comb:
                    ...
                    for i_i in range(len_c):
                        ...
                        a_ij = 0.0
                        q_i_func = q_func_name[i_i]
                        q_i_norm = 0.0
                        for i in range(l_sph2p1):
                            q_i_norm += (q_i_func[i]*q_i_func[i].conjugate()).real
                        q_i_norm = math.sqrt(q_i_norm)
                        for i_j in ineighbor_list[i_i]:
                            q_j_func = q_func_name[i_j]
                            q_j_norm = 0.0
                            for j in range(l_sph2p1):
                                q_j_norm += (q_j_func[j]*q_j_func[j].conjugate()).real
                            q_j_norm = math.sqrt(q_j_norm)

                            a_temp = 0.0
                            for j in range(l_sph2p1):
                                a_temp += (q_i_func[j]*q_j_func[j].conjugate()).real/(q_i_norm * q_j_norm)
                            a_ij += a_temp

                        if l_neighbor_list_ii != 0:
                            a_ij = a_ij / <double>l_neighbor_list_ii
                        else:
                            a_ij = 0.0
            ...
            for b_t in range(b_times + 1):
                for o_f, o_i, o_j, p_weight in comb:
                    ...
                    for i_i in range(len(coord)):
                        ...
                        ## q_i_func = [q_func[name][i_i][i] for i in range(l_sph2p1)]
                        q_i_func = q_func[name][i_i]
                        ## q_i_norm = calc_q_norm(l_sph, q_i_func)
                        q_i_norm = 0.0
                        for i in range(l_sph2p1):
                            q_i_norm += (q_i_func[i]*q_i_func[i].conjugate()).real
                        q_i_norm = math.sqrt(q_i_norm)
                        ## q_ij_func = [0 for i in range(l_sph2p1)]
                        q_ij_func = cvec()
                        for i in range(l_sph2p1):
                            q_ij_func.push_back(0+0j)
                        for i_j in ineighbor_list[i_i]:
                            ## q_j_func = [q_func[name][i_j][i] for i in range(l_sph2p1)]
                            q_j_func = q_func[name][i_j]
                            ## q_i_norm = calc_q_norm(l_sph, q_i_func)
                            ## q_j_norm = calc_q_norm(l_sph, q_j_func)
                            q_j_norm = 0.0
                            for j in range(l_sph2p1):
                            ...
                            q_j_norm = math.sqrt(q_j_norm)
                            for i in range(l_sph2p1):
                                ## q_ij_func[i] += (q_i_func[i]*np.conjugate(q_j_func[i]))/(q_i_norm * q_j_norm)
                                q_ij_func[i] += (q_i_func[i]*q_j_func[i].conjugate())/(q_i_norm * q_j_norm)
                        ## if len(neighbor_list[i_i]) != 0:
                        if l_neighbor_list_ii != 0:
                            ## q_ij_func = [q_ij_func[i] / float(len(neighbor_list[i_i]))
                            ##              for i in range(l_sph2p1)]
                            for i in range(l_sph2p1):
                                q_ij_func[i] = q_ij_func[i] / <double complex>l_neighbor_list_ii
                        else:
                            ## q_ij_func = [q_i_func[i] for i in range(l_sph2p1)]
                            q_ij_func = q_i_func
                        ## w_val.append(func_to_value_wigner(l_sph, wigner3j, q_ij_func))
                        sum_vec = 0.0
                        for i in range(lwigner3j):
                            sum_vec += dwigner3j[i]* \
                            (q_ij_func[im1[i]]*q_ij_func[im2[i]]*q_ij_func[im3[i]]).real
                        sum_norm2 = 0.0
                        for i_j in range(-l_sph, l_sph + 1):
        ...
        for l_sph in l_list:
            for b_t in range(b_times + 1):
                for o_f, o_i, o_j, p_weight in comb:
                    ...
                    for i_i in range(len(coord)):
                        # calc spherical harmonics function i-j
                        a_ij = calc_qi_qj(q_func, name, i_i,
                                          neighbor_list, l_sph)
                        q_val.append(a_ij)
                    op_data[name] = q_val
    elif calc_type == 'W':
        for l_sph in l_list:
            ...
            for b_t in range(b_times + 1):
                for o_f, o_i, o_j, p_weight in comb:
                    ...
                    for i_i in range(len(coord)):
                        # calc spherical harmonics function i-j
                        q_ij_func = calc_qi_qj_func(
                            q_func, name, i_i, neighbor_list, l_sph)
                        w_val.append(func_to_value_wigner(
                            l_sph, wigner3j, q_ij_func))
                    op_data[name] = w_val
