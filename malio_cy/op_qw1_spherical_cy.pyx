...
def calc_q_wrapper(args):
    [box_length, neighbor_list_ii,
        i_i, l_list, nei_area, o_factor, oi_oj, p_list] = args
    ...
    for l_sph, o_f, o_i, o_j, p_weight in comb:
        ...
        for i in range(len(neighbor_list_ii)):
            ...
            q_l = calc_q_weighted(l_sph, pol['theta'], pol['phi'], area_weight)
        ...
        for i_j in oi_list_not_oi:
            ...
            pol = misc.convert_to_theta_phi(
                delta)
            q_l = calc_q_weighted(l_sph, pol['theta'], pol['phi'], 1)
...
def calc_spherical_order_parameter(calc_type, coord, direct, box_length, setting, neighbor_list, neighbor_area, thread_num):
    ...
    cdef:
        ...
        double area_weight, sum_area, num_nei

    q_func_temp= []
    ## [box_length, neighbor_list[i_i], i_i, l_list, neighbor_area[i_i], o_factor, oi_oj, p_list]
    ## [box_length, neighbor_list_ii,i_i, l_list, nei_area, o_factor, oi_oj, p_list]
    for i_i in range(len_c):

        l_neighbor_list_ii = len(neighbor_list[i_i])
        ## sum_area = np.sum(neighbor_area[i_i])
        sum_area = 0.0
        for i in range(len(dneighbor_area[i_i])):
            sum_area += dneighbor_area[i_i][i]
        num_nei = <double>l_neighbor_list_ii
        comb = [(l_sph, o_f, o_i, o_j, p_weight)
                for l_sph in l_list for o_f in o_factor
                for o_i in oi_oj for o_j in oi_oj for p_weight in p_list]
        ...
        for l_sph, o_f, o_i, o_j, p_weight in comb:
            ...
            ## sum_area = np.sum(neighbor_area[i_i])
            ## num_nei = len(neighbor_list[i_i])
            ## for i in range(len(neighbor_list[i_i])):
            for i_2 in range(l_neighbor_list_ii):
                ## i_j = neighbor_list[i_i][i]
                ...
                ## area_weight = num_nei*(neighbor_area[i_i][i]/sum_area)
                area_weight = num_nei*(dneighbor_area[i_i][i]/sum_area)
                ## q_l = calc_q_weighted(l_sph, pol['theta'], pol['phi'], area_weight)
                ## for i_k in range(2 * l_sph + 1):
                ##     q_temp[i_k] += q_l[i_k]
                q_temp[0] += area_weight*sph_harm_cy(l_sph, 0, theta, phi)
                for m_sph in range(1, l_sph + 1):
                    sphharm = sph_harm_cy(l_sph, m_sph, theta, phi)
                    q_temp[m_sph] += area_weight*sphharm
                    q_temp[l_sph2p1-m_sph] += area_weight*sphharm
            ...
            for i_j in oi_list_not_oi:
                ...
                ##        for i in range(3)]
                for i in range(3):
                    x_j[i] = dcoord[i_i][i] + direct[i_i][i]
                ## delta = misc.calc_delta(x_i, x_j, box_length)
                for i in range(3):
                    delta[i] = x_i[i] - x_j[i]
                    if delta[i] < -dbox_length[i] * 0.5:
...
def calc_spherical_order_parameter_org(calc_type, coord, direct, box_length, setting, neighbor_list, neighbor_area, thread_num):
    ...
    args = [[box_length, neighbor_list[i_i], i_i, l_list, neighbor_area[i_i], o_factor, oi_oj, p_list]
