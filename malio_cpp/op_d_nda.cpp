...
map<string, double> op_d_nda::calc_d_wrapper(const vector<Vector3>& coord,
          ...
          for(int o_j : settings.oi_oj_ok) {
            for(int o_k : settings.oi_oj_ok) {
              for(int o_f : settings.o_factor) {
                string name = misc::naming_d(0, o_f, o_i, o_j, o_k, f_1, f_2, f_3);
                ...
                for( int i_2 = 0; i_2 < N; ++i_2) {
                  ...
                  for(int i_3 = i_2 + 1; i_3 < N; ++i_3) {
                    ...
                    Vector3 x_j_k = misc::calc_delta(x_k, x_j, box_length);
                    double d_i_k = x_i_k.Norm();
                    double d_j_k = x_j_k.Norm();

                    d_sum += func[f_1](d_i_j) * func[f_2](d_i_k) * func[f_3](d_j_k);
                  }
                }
...
map<string, vector<double>> op_d_nda::nda_order_parameter(const vector<Vector3>& coord,
                                                          const vector<Vector3>& direct,
                                                          const Vector3& box_length,
                                                          const D_SETTINGS& settings,
                                                          const vector<vector<int>>& neighbor_list)
{
  ...
  for(int i = 0; i < n_coord; ++i) {
    map<string, double> op_temp = calc_d_wrapper(coord, direct, box_length, neighbor_list[i], i, settings);
        ...
        for(int f_1 = 0; f_1 < nf; ++f_1) {
          for( int o_f : settings.o_factor ) {
            for( int o_k : settings.oi_oj_ok ) {
              for( int o_j : settings.oi_oj_ok ) {
                for( int o_i : settings.oi_oj_ok ) {
                  string name = misc::naming_d(a_t + 1, o_f, o_i, o_j, o_k, f_1, f_2, f_3);
                  string name_old = misc::naming_d(a_t, o_f, o_i, o_j, o_k, f_1, f_2, f_3);
