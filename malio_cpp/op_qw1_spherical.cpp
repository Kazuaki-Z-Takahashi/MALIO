...
double op_qw1_spherical::func_to_value_wigner(int l_sph, const vector<vector<vector<double>>>& wigner3j,
...
vector<vector<vector<double>>> op_qw1_spherical::gen_wigner3j(int l_sph)
{
  int l2 = 2 * l_sph + 1;
...
COMPLEX op_qw1_spherical::sph_harm(int l_sph, int m_sph, double theta, double phi)
{
  return SphericalHarmonics::Y(l_sph, m_sph, theta, phi);
}

vector<COMPLEX> op_qw1_spherical::calc_q_weighted(int l_sph, double theta, double phi, double weight)
{
  int n = 2 * l_sph + 1;
...
double op_qw1_spherical::func_to_value(int l_sph, const vector<COMPLEX>& func)
{
  double sum_norm2 = 0.0;
  for( int i_j = 0; i_j < 2 * l_sph + 1; ++i_j) {
    COMPLEX comp = func[i_j];
...
map<string, vector<COMPLEX>> op_qw1_spherical::calc_q_wrapper(const vector<Vector3>& coord,
      ...
      for( int o_i : setting.oi_oj ) {
        for( int o_f : setting.o_factor ) {
          for( int l_sph : setting.l_in_Q ) {
            ...
            double sum_area = 0.0;
            for(double d : nei_area) {
              sum_area += d;
            }
              
            int num_nei = neighbor_list_ii.size();
            for( int i = 0; i < num_nei; ++i ) {
              int i_j = neighbor_list_ii[i];
              ...
              double area_weight = num_nei * (nei_area[i] / sum_area);
              vector<COMPLEX> q_l = calc_q_weighted(l_sph, theta, phi, area_weight);
            ...
            for( int i_j : oi_list_not_oi ) {
              ...
              double phi = 0.0;
              misc::convert_to_theta_phi(delta, theta, phi);
              vector<COMPLEX> q_l = calc_q_weighted(l_sph, theta, phi, 1);
...
op_qw1_spherical::calc_spherical_order_parameter(char calc_type,
...
map<string, vector<double>> op_qw1_spherical::spherical_order_parameter(const vector<Vector3>& coord,
                                                                        const vector<Vector3>& direct,
                                                                        const Vector3& box_length,
                                                                        const QW1_SETTINGS& setting,
                                                                        const vector<vector<int>>& neighbor_list,
...
map<string, vector<double>> op_qw1_spherical::w_order_parameter(const vector<Vector3>& coord,
                                                                const vector<Vector3>& direct,
                                                                const Vector3& box_length,
                                                                const QW1_SETTINGS& setting,
                                                                const vector<vector<int>>& neighbor_list,
